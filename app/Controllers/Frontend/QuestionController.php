...
class QuestionController extends BaseController
{
    ...
    public function submit(Request $request): Response
    {
        $user    = Session::get('auth_user');
        $eventId = (int) $request->param('eventId');
        $text    = trim($request->input('question', ''));
        ...
        $id = Database::insert(
            'INSERT INTO questions (event_id, user_id, question, status, created_at)
             VALUES (?, ?, ?, \'pending\', NOW())',
            [$eventId, $user['id'], $text]
        );

        return Response::json(['id' => $id, 'message' => 'Question submitted.'], 201);
    }
    ...
    public function upvote(Request $request): Response
    {
        ...
        if ($exists) {
            ...
            Database::insert(
                'INSERT INTO question_upvotes (question_id, user_id, created_at) VALUES (?, ?, NOW())',
                [$questionId, $user['id']]
            );
            $upvoted = true;
        }
